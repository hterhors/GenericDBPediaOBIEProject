...
public class NumberOfVolumesInterpreter extends AbstractNumericInterpreter {
	/**
	 * 
	 */
	...
	private NumberOfVolumesInterpreter(String surfaceForm, int value) {
		super(surfaceForm);
		this.value = value;
	}

	public NumberOfVolumesInterpreter normalize() {
		return this;
	}
	...
	public static class Builder extends AbstractInterpreterBuilder {
		...
		public NumberOfVolumesInterpreter build() {
			return new NumberOfVolumesInterpreter(surfaceForm, value);
		}

		public Builder interprete(final String surfaceForm) {
			Matcher matcher = NumberOfVolumesInterpreter.PATTERN.matcher(surfaceForm);
			if (!matcher.find())
				return this;
		...
		public Builder fromMatcher(final Matcher matcher) {
			...
			if (matcher.group(NumberOfVolumesInterpreter.pattern1GroupName) != null) {
				if (matcher.group(NumberOfVolumesInterpreter.numGroup1) != null)
					value = Integer.parseInt(matcher.group(NumberOfVolumesInterpreter.numGroup1));
				else if (matcher.group(NumberOfVolumesInterpreter.writtenNumGroup1) != null)
					value = (int) mapWrittenNumbertoInt(matcher.group(NumberOfVolumesInterpreter.writtenNumGroup1));
			}
			return this;
		}

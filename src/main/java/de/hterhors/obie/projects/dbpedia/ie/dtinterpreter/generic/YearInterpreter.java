package de.hterhors.obie.projects.dbpedia.ie.dtinterpreter.generic;
...
public class YearInterpreter extends AbstractNumericInterpreter {
	/**
	 * 
	 */
	...
	private YearInterpreter(String surfaceForm, int value) {
		super(surfaceForm);
		this.value = value;
	}

	public YearInterpreter normalize() {
		return this;
	}
	...
	public static class Builder extends AbstractInterpreterBuilder {
		...
		public YearInterpreter build() {
			return new YearInterpreter(surfaceForm, value);
		}

		public Builder interprete(final String surfaceForm) {
			Matcher matcher = YearInterpreter.PATTERN.matcher(surfaceForm);
			if (!matcher.find())
				return this;
		...
		public Builder fromMatcher(final Matcher matcher) {
			...
			if (matcher.group(YearInterpreter.pattern1GroupName) != null) {
				if (matcher.group(YearInterpreter.numGroup1) != null)
					value = Integer.parseInt(matcher.group(YearInterpreter.numGroup1));
				else if (matcher.group(YearInterpreter.writtenNumGroup1) != null)
					value = (int) mapWrittenNumbertoInt(matcher.group(YearInterpreter.writtenNumGroup1));
			}
			return this;
		}

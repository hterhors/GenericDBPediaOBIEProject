...
public class Food implements IFood {
	...
	static class FoodIndividual extends AbstractIndividual {
		...
		public FoodIndividual(String namespace, String name) {
	...
	final static public String ONTOLOGY_NAME = "http://dbpedia.org/ontology/Food";
	...
	public Food(String individualURI, String textMention) {
		this.individual = Food.individualFactory.getIndividualByURI(individualURI);
		this.textMention = textMention;
	}

	public Food(Food food) throws InstantiationException, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, NoSuchMethodException, SecurityException {
		this.individual = food.individual;
		if (food.getAlias() != null)
			this.alias = new Alias((Alias) food.getAlias());
		this.characterOffset = food.getCharacterOffset();
		this.characterOnset = food.getCharacterOnset();
		for (int j = 0; j < food.getCreatorOfDishPersons().size(); j++) {
			if (food.getCreatorOfDishPersons().get(j) != null) {
				creatorOfDishPersons
						.add((IPerson) IOBIEThing.getCloneConstructor(food.getCreatorOfDishPersons().get(j).getClass())
								.newInstance(food.getCreatorOfDishPersons().get(j)));
			} else {
				creatorOfDishPersons.add(null);
			}
		}
		for (int j = 0; j < food.getIngredientEukaryotes().size(); j++) {
			if (food.getIngredientEukaryotes().get(j) != null) {
				ingredientEukaryotes.add(
						(IEukaryote) IOBIEThing.getCloneConstructor(food.getIngredientEukaryotes().get(j).getClass())
								.newInstance(food.getIngredientEukaryotes().get(j)));
			} else {
				ingredientEukaryotes.add(null);
			}
		}
		for (int j = 0; j < food.getOriginPopulatedPlaces().size(); j++) {
			if (food.getOriginPopulatedPlaces().get(j) != null) {
				originPopulatedPlaces.add((IPopulatedPlace) IOBIEThing
						.getCloneConstructor(food.getOriginPopulatedPlaces().get(j).getClass())
						.newInstance(food.getOriginPopulatedPlaces().get(j)));
			} else {
				originPopulatedPlaces.add(null);
			}
		}
		this.textMention = food.getTextMention();
		for (int j = 0; j < food.getVariantFoods().size(); j++) {
			if (food.getVariantFoods().get(j) != null) {
				variantFoods.add((IFood) IOBIEThing.getCloneConstructor(food.getVariantFoods().get(j).getClass())
						.newInstance(food.getVariantFoods().get(j)));
			} else {
				variantFoods.add(null);
			}
		}
	}
	...
	public Food setCreatorOfDishPersons(List<IPerson> persons) {
		...
		this.creatorOfDishPersons = persons;
		return this;
	}
	...
	public Food setIngredientEukaryotes(List<IEukaryote> eukaryotes) {
		...
		this.ingredientEukaryotes = eukaryotes;
		return this;
	}
	...
	public Food setOriginPopulatedPlaces(List<IPopulatedPlace> populatedPlaces) {
		...
		this.originPopulatedPlaces = populatedPlaces;
		return this;
	}
	...
	public Food setVariantFoods(List<IFood> foods) {
		...
		this.variantFoods = foods;
		return this;
	}

	@Override
	public String toString() {
		return "Food [individual=" + individual + ",alias=" + alias + ",characterOffset=" + characterOffset
				+ ",characterOnset=" + characterOnset + ",creatorOfDishPersons=" + creatorOfDishPersons
				+ ",ingredientEukaryotes=" + ingredientEukaryotes + ",originPopulatedPlaces=" + originPopulatedPlaces
				+ ",serialVersionUID=" + serialVersionUID + ",textMention=" + textMention + ",variantFoods="
				+ variantFoods + "]";
	}
